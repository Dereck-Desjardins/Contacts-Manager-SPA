const API_BOOKMARK_URL = "http://localhost:5000/api/bookmarks";
function API_GetBookmarks() {
    return new Promise(resolve => {
        $.ajax({
            url: API_BOOKMARK_URL,
...
function API_GetBookmark(bookmarkId) {
    return new Promise(resolve => {
        $.ajax({
            url: API_BOOKMARK_URL + "/" + bookmarkId,
...
function API_SaveBookmark(bookmark, create) {
    return new Promise(resolve => {
        $.ajax({
            url: API_BOOKMARK_URL + (create ? "" : "/" + bookmark.Id),
